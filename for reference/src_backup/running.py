...
def pipeline_factory(config):
    ...
    elif (task == "dynamic_classification"):
...
class UnsupervisedRunner(BaseRunner):
    ...
    def train_epoch(self, epoch_num=None):
        ...
        for i, batch in enumerate(self.dataloader): # I added label check to monitor SOZ distribution
    ...
    def evaluate(self, epoch_num=None, keep_all=True):
        if isinstance(self.model, torch.nn.Module):
            # Update BatchNorm running stats on a few batches first
            self.model.train()
            with torch.no_grad():
                for i, batch in enumerate(self.dataloader):
                    if i >= 5:  # Update on 5 batches
                        break
                    X, targets, target_masks, padding_masks, IDs = batch
        ...
        for i, batch in enumerate(self.dataloader):
            ...
            with torch.no_grad():
                ...
                if i == 0:
                    print("\nReconstruction Accuracy Debugging:")
                    print(f"Predictions range: [{predictions.min():.4f}, {predictions.max():.4f}]")
                    print(f"Targets range: [{targets.min():.4f}, {targets.max():.4f}]")
                    
                    # Check mask statistics
                    print(f"\nMask statistics:")
                    print(f"Target masks sum: {target_masks.sum()}")
                    ...
                    print(f"\nMasked values statistics:")
                    print(f"Masked predictions range: [{masked_pred.min():.4f}, {masked_pred.max():.4f}]")
                    print(f"Masked targets range: [{masked_true.min():.4f}, {masked_true.max():.4f}]")
                    
                    # Calculate and print error distribution
                    errors = torch.abs(masked_pred - masked_true)
                    print(f"\nError statistics:")
                    ...
                    rel_threshold = 0.1 * data_range
                    print(f"\nThreshold calculation:")
                    print(f"Data range: {data_range:.4f}")
                    print(f"Relative threshold: {rel_threshold:.4f}")
                    
                    # Print accuracy details
                    acc = (errors < rel_threshold).float()
                    print(f"\nAccuracy details:")
                    print(f"Number of points within threshold: {acc.sum()}/{len(acc)}")
                ...
                # Calculate reconstruction accuracy with relative threshold
                masked_pred = torch.masked_select(predictions, target_masks)
                masked_true = torch.masked_select(targets, target_masks)
                
                # Use normalized error threshold
                errors = torch.abs(masked_pred - masked_true)
                error_threshold = 0.1  # 10% of normalized range
                batch_recon_acc = torch.mean((errors < error_threshold).float())
