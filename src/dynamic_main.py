...
# only add accuracy metrics if it's a classification task
    ...
    val_loader = DataLoader(
        ...
        shuffle=False,
        num_workers=config['num_workers'], # difference between n_proc and num_workers?
    ...
    trainer = runner_class(model, train_loader, device, loss_module, optimizer, l2_reg=output_reg, # what is output_reg?
                           print_interval=config['print_interval'], console=config['console'])
    
    val_evaluator = runner_class(model, val_loader, device, loss_module, optimizer=None, 
                                 l2_reg=config['l2_reg'] if 'l2_reg' in config else None, # why need l2_reg for validation?
    ...
    best_value = 1e16 if config['key_metric'] in NEG_METRICS else -1e16 # initialize with +inf or -inf depending on key metric, clarify which is which?
    metrics = []
    best_metrics = {}
